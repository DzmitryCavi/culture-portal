export const worklogTables = [
  {
    id: 0,
    name: 'Dzmitry Malakhouski',
    performedWork: [
      {timeSpent: '5', performedWork: 'performedWork1'},
    ]
  },
  {
    id: 1,
    name: 'Vasiliy Pisarenka',
    performedWork: [
      {timeSpent: '4', performedWork: 'performedWork1'},
      {timeSpent: '3', performedWork: 'performedWork2'},
    ]
  },
  {
    id: 2,
    name: 'Yura Vaskevich',
    performedWork: [
      {timeSpent: '5', performedWork: 'performedWork1'},
      {timeSpent: '4', performedWork: 'performedWork2'},
      {timeSpent: '10', performedWork: 'performedWork3'},
      {timeSpent: '6', performedWork: 'performedWork4'},
      {timeSpent: '8', performedWork: 'performedWork5'},
      {timeSpent: '6', performedWork: 'performedWork6'},
    ]
  },
  {
    id: 3,
    name: 'Yury Zikeyeu',
    performedWork: [
      {timeSpent: '7', performedWork: 'performedWork1'},
      {timeSpent: '3', performedWork: 'performedWork2'},
      {timeSpent: '4', performedWork: 'performedWork3'},
      {timeSpent: '4', performedWork: 'performedWork4'},
      {timeSpent: '3', performedWork: 'performedWork5'},
    ]
  },
  {
    id: 4,
    name: 'Aldar Antonov',
    performedWork: [
      {timeSpent: '2', performedWork: 'performedWork1'},
      {timeSpent: '2', performedWork: 'performedWork2'}
    ]
  },
]